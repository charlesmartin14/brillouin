...
By Singlet-like states, we effectively mean total nuclear spin 0, and by Triplet-like, we mean total nuclear spin $1$ or $-1$.  Since we are using particle-in-the-box electronic wave functions,  we do not consider orbital angular momentum, and therefore only have to consider if $s_{n}=s_{\nu}$. Notice, however, we use the Dirac-spinor convention for $s=1\;(up)$ and $s=2\;(down)$, so total $s=0$ corresponds to ($up,down$) or ($down,up$), etc.
