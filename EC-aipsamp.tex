...
We pose the following Gedankenexperiment:   We imagine some protons embedded in lattice, such that we can say each bare proton is confined in a Fermi sea of electrons. We model this as a classical particle-in-a-box, weith volume $L^3$.   We further imagine that the box is transiently compressed, as in Figure \ref{fig:box1}, such that the box size L is just small enough to  'induce' electron capture.  
...
For example, to estimate electron capture rates for something  $^7Be$ confined in a cage, we assume the cross section is not changing, and estimate rate by computing the ratio of the confined to uncaged classical molecular electronic densities. 

$$\Gamma^{confined}_{EC}\sim\dfrac{\vert\Psi^{confined}_{e}(0)\vert^{2}}{\vert\Psi^{free}_{e}(0)\vert^{2}}\Gamma^{free}_{EC}$$

\subsection{Relativistic Cross Sections}

In our Gedankenexperiment, however, we imagine  box confinement induce capture, and, therefore, we are at least in the realm of relativistic kinematics. To that end,
