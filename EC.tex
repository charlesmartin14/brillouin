...
In 1935, Yukawa proposed that a proton, bound in an atomic nucleus,  could capture a low lying, bound atomic electron, transforming into a neutron, and releasing an electron neutrino \cite{yukawa}.
...
Electron capture usually occurs in unstable radioisotopes which lack the (nuclear binding) energy to decay by the more familiar Beta decay (positron emission).  In particular, heavy elements may decay by E.C. and/or $\beta^{+}$ to a lower 'magic number' of stable nuclei, and by $\beta$ decay to achieve a higher magic number.  Very light elements, such as $^{7}Be$,  may also decay by E.C.
  
We observe electron capture by observing the resulting transmuted nuclei and/or the radiative relaxation processes.  The captured electron is bound to the atom, and it is usually a K-shell electron, but may be L or higher.  The nucleus may then absorb some energy, becoming excited, and then undergo internal conversion.  During this, another higher lying, bound atomic electron is absorbed, and an X-ray, or Auger electron released [see Figure \ref{fig:ec1}].
...
The lightest element that electron capture has been observed in is $^{7}Be$\cite{radbook}. In fact, there is so little energy that the competing $\beta^{+}$ positron emission process (described below) is prohibited, and the half-life for electron capture is $\tau_{EC}\sim 50\;days$.

Being so light, and having such a large rate, electron capture in $^{7}Be$ can be slightly modified by both changing the chemical environment and/or the external pressure \cite{wang,raydas,ohtsuki}. In particular, in 2004, Ohtsuki et. al. demonstrated a change of $0.83\%$  by embedding Be in C-60 cages.  

How could such changes occur?

The rate of electron capture depends the electronic density at the surface of the nucleus--the nuclear charge.   The electronic energy levels are in the eV range, so intense EM fields can alter the electronic structure and therefore slightly affect the rate. In contrast, the nuclear energy levels are in the keV to MeV region, and it is generally thought to be very difficult to impossible to effect. Of particular practical interest is using very large electric fields for accelerating Beta decay for disposing of nuclear waste \cite{iaea, reiss}.
...
Electron capture is also sometimes called inverse $\beta$ decay, but, here, we mean this to be the scattering of a proton and an electron anti-neutrino.  It is characterized by emission of a positron $e^{+}$. 
...
Bare proton-electron capture is thought to occur  in the early stages of the big bang and in stellar nucleosynthesis .  It is thought to drive the formation of primordial elements, and to occur in the forming of neutron stars [?].   For example, Bachall and coworkers have studied the electron capture rate in stellar media, and have computed the electron capture rate of $^{7}Be$ in the Sun \cite{bachall62,bachall69}.
...
We can safely use a classical box as long as $L_{min} \ge \dfrac{1}{2\pi}\lambda_{e}$, where $\lambda_{e}$ is Compton wavelength of an electron \cite{relbox,compton,planck}.  The Compton wavelength sets the scale, accounting for both quantum mechanics and special relativity.
...
which we argue is a good measure of the potential reactive power output of the confined E.C. process.


\subsubsection{Particle-in-the-box Wavefunctions}

While most E.C. calculations assume a specific, bound, atomic electronic wavefunction(s), we perform a much simpler calculation.  We treat the electron-proton pair as classical particle-in-a-box. 
...
where $J_{\mu}$ and $L_{\mu}$ are (in modern parlance) the Hadron and Lepton currents, resp., and are given by
...
\subsection{Numerical Calculations}
 ...
 The full calculations sums of over all 6 possible incident momenta combinations
  $(\pm\mathbf{p}_{ep}(1),\pm\mathbf{p}_{ep}(2),\pm\mathbf{p}_{ep}(3) )$,
and the 16 allowed spin transitions (see figure below). By conservation of momenta, we can eliminate the neutron and only need to average over the outbound neutrino momenta solid angle ($k^{3}d\Omega_k$); we do this using an 8-point gaussian quadrature. 
...
\For{$s_{p},s_{e},s_{n},s_{\nu}\in [0, 1]$}\Comment{$s_{up},s_{down}=0,1$}
\For{$s_{x},s_{y},s_{z}\in [-1,1]$}\Comment{$\mathbf{p}_{ep}=\mathsf{p}_{ep}[s_{x},s_{y},s_{z}]$}
\State $p_{p},p_{e}, p_{n}, p_{\nu}\gets pekin(w_{x},w_{\phi},\mathbf{p}_{ep})$\Comment{4 vectors $\gets$ rel. kinematics}
\State $u(p_{p},s_{p}), u(p_{e},s_{e}), \bar{u}(p_{n},s_{n}), \bar{u}(p_{\nu},s_{\nu})$\Comment{spinors by p, s-index}
\For{$u\in[1,4]$}\Comment{4-vector indices}
\State $\mathbf{J}^{u}_{had}=\bar{u}(p_{n},s_{n})\cdot(G_{V}-G_{A}\gamma^{5})\cdot\gamma^{\mu}\cdot u(p_{p},s_{p})$\Comment{Hadronic current}
\State $\mathbf{L}^{u}_{lep}=\bar{u}(p_{\nu},s_{\nu})\cdot\gamma_{u}\cdot(1-\gamma^{5})\cdot u(p_{e},s_{e})$\Comment{Leptonic current}
\EndFor
...
We sum over the 16  allowed transitions in the V-A theory, as depicted in Figure

[similar to diagram in "Beta Decay for Pedestrians"]

\subsubsection{Numerical Quadrature over neutrino solid angle}

By Energy conservation, we only need to integrate over the outbound neutrino momenta (solid angle).
We use an 8-point numerical quadrature.

\begin{figure}
   \includegraphics[scale=0.1]{img/solid-angle.png}
   \caption{Solid Angle }
  \label{fig:NQ}
\end{figure}

[this diagram needs to be replaced, with something that looks more like a scattering diagram]
...
[references  , not correctly numbered]


\begin{thebibliography}{21}

\bibitem{yukawa} Yukawa and Sakata, \emph{On the Nuclear Transformation with the Absorption of the Orbital Electron },
Phys. Rev. 51, 677 (1937).  

\bibitem{alvarez} Luis W. Alvarez, \emph{The Capture of Orbital Electrons by Nuclei}, Phys. Rev. 54, 486 (1938).

\bibitem{wang} B. Wang; et al., \emph{Change of the $^{7}Be$ electron capture half-life in metallic environments},The European Physical Journal A, 28, 375-Ð377 (2006). 

\bibitem{raydas}A. Ray, P. Das, S. K. Saha, A. Goswami, A. De, \emph{Enhanced orbital electron-capture nuclear decay rate in compact medium}, Physics Letters B, 679, 106--110 (2009).

\bibitem{ohtsuki}T. Ohtsuki, H. Yuki, M. Muto, J. Kasagi, and K. Ohnol, \emph{Enhanced Electron-Capture Decay Rate of Be-7 Encapsulated in C-60 Cages}. Phys. Rev. Lett. 93(11), 112501 (2004)

\bibitem{ec-review1}W. Bambynek, H. Behrens, M. H. Chen, R. L. Intemann,  \emph{Orbital electron capture by the nucleus},  Review of Modern Physics,  49(1) (1976)

\bibitem{ec-review2}R. Bouchez and P. Deponimier,  \emph{Orbital electron capture by the nucleus},  Rep. Prog. Phys., 23, 395 (1960)

\bibitem{glauber1}R. J. Glauber and P. C. Martin,  \emph{Radiative Capture of Orbital Electrons},  Phys. Rev. 104, 158 (1956)

\bibitem{glauber2}R. J. Glauber and P. C. Martin,  \emph{Relativistic Theory of Radiative Orbital Electron Capture}, Phys. Rev. 109, 1307 (1958)

\bibitem{roec2007}K. Pachucki and U.D. Jentschura, \emph{Radiative orbital electron capture by the atomic nucleus}, Phys. Rev. C, 75, 055502 (2007)

\bibitem{bachall62}John N. Bahcall, \emph{Electron Capture and Nuclear Matrix Elements of $^{7}Be$}, Phys. Rev. 128, 1297 (1962)

\bibitem{bachall69}Andrei V. Gruzinov and John N. Bahcall, \emph{The $^{7}Be$ electron capture rate in the sun}, Astrophysical Journal,  155, 511 (1969)

\bibitem{relbox}P Alberto, C Fiolhais and V M S Gil, \emph{Relativistic particle in a box}. Eur. J. Phys., 17, 19--24 (1996)

\bibitem{compton}Garay, Luis J., \emph{Quantum Gravity And Minimum Length}, Int. J. Mod. Phys., A10, 145 (1995)

\bibitem{planck}B.G. Sidharth, \emph{Planck scale to Compton scale}  (2006) 

\bibitem{axial} J. Liu et. al., \emph{Determination of the Axial-Vector Weak Coupling Constant with ultracold neutrons} (2010)

\bibitem{wheeler}J. A.Wheeler, \emph{Polyelectrons}, Ann. NY Acad. Sci. 48, 219Ð238 (1946)

\bibitem{penning1} S. F. Hoogerheide et. al., \emph{Experiments with Highly-Ionized Atoms in Unitary Penning Traps}, Atoms, 3, 367--391 (2015)

\bibitem{brem1}Jo Me Jauch , \emph{On the theory of Inner Bremsstrahlung in $\beta$ processes}, ORNL 1102 Report (1951)

\bibitem{penning2}T Brunner1, \emph{Design of a ?-detector for TITAN-EC and the first electron-capture branching ratio measurement in a Penning trap},J. Phys.: Conf. Ser., 312, 072006 (2011)

\bibitem{radbook},Michael F. L'Annunziata, \emph{Radioactivity: Introduction and History, From the Quantum to Quarks}, Second Edition, Elsevier, (2016)

\bibitem{jackson},John D. Jackson, \emph{Classical Electrodynamics},  Third Edition, Wiley, (1998)

\bibitem{iaea}International Atomic Energy Agency,  \emph{Methods for the Minimization of Radioactive Waste from Decontamination and Decommissioning of Nuclear Facilities}, Vienna (2001)

\bibitem{reiss}Howard R. Reiss, \emph{Accelerated Beta Decay for the Disposal of Fission Fragment Wastes}

\bibitem{close}Frank Close, \emph{Small Things and Nothing}, OUP Oxford (2013)

\bibitem{fermi1}E. Fermi, \emph{An attempt of a theory of beta radiation.1.}, Z. Phys.,  88, 161-177 (1934) 

\bibitem{Jauch}Jo Me Jauch, \emph{On the Theory of Inner Bremsstrahlung in $\beta$ decay}, Oak Ridge National Laboratory, Contract No, W-7405, eng 26 (1951)

\bibitem{martin}C. H. Martin and M. C. Zerner,  \emph{Electronic Structure Calculations on Transition Metal Complexes: Ab Initio Models and Approximate Models}, Inorganic Electronic Structure and Spectroscopy, Volume I: Methodology, E. I. Solomon and A. B. P. Lever, Wiley (2006)

\bibitem{Weisskopf}J. M. Blatt and V. F. Weisskopf, \emph{Theoretical Nuclear Physics}, Dover Books on Physics

\bibitem{Sen}\emph{Electronic Structure of Quantum Confined Atoms and Molecules},  edited by K.D. Sen

\bibitem{feynman}Feynman and Gelmann
