...
\begin{algorithmic}[1]
\For{$p_{ep}\in [p_{max}, p_{min}]$}\Comment{incident momenta}
\State $Tran(p_{ep})=0$\Comment{Transfer}
\State $P_{n}(p_{ep})=0$\Comment{Neutron Power}
\For{$w_{x},w_{\phi}\in \mathbb{Q}[1,8]$}\Comment{quadrature weights}
\For{$s_{p},s_{e},s_{n},s_{\nu}\in [up, down]$}\Comment{spinor indices}
\For{$s_{x},s_{y},s_{z}\in [-1,1]$}\Comment{$\mathbf{p}_{ep}=p_{ep}[s_{x},s_{y},s_{z}]$}
\State $P_{n}, P_{p},P_{e}, P_{\nu}\gets pekin(w_{x},w_{\phi},\mathbf{p}_{ep})$\Comment{relativistic 4-vectors}
\State $u(p,s_{p}), u(e,s_{e}), \bar{u}(n,s_{n}), \bar{u}(\nu,s_{\nu})$\Comment{get spinors by index}
\For{$u\in[1,4]$}\Comment{Hamiltonian Currents}
\State $\mathbf{J}^{u}_{had}=\bar{u}(p_{n},s_{n})\cdot(G_{V}-G_{A}\gamma^{5})\cdot\gamma^{\mu}\cdot u(p_{p},s_{p})$\Comment{Hadronic}
\State $\mathbf{L}^{u}_{lep}=\bar{u}(k,s_{k})\cdot\gamma_{u}\cdot(1-\gamma^{5})\cdot u(s_{e},p_{e})$\Comment{Leptonic}
\EndFor
\State $\mathbb{C}=\langle\mathbf{L}^{\dagger}_{lep},\mathbf{J}_{had}\rangle$\Comment{Current Amplitude}
\EndFor
\EndFor
\State $Tran(p_{ep})+=w_{x}w_{\phi}Re[\mathbb{C}^{\dagger}\mathbb{C}]\mathcal{M}^{2}(p_{ep},P_{n}, P_{p},P_{e}, P_{\nu})$\Comment{Integrated Rate}
\State $P_{n}(p_{ep})=Tran(p_{ep}*(2.2+(E_{n}-M_{n}))$\Comment{Neutron Power estimate}
\EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
